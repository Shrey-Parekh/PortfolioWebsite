...
function App() {
  ...
  const openWindow = useCallback(
    (windowType: string, sourceElement?: HTMLElement) => {
            ...
            w.id === existingWindow.id
              ? {
                  ...
                  sourcePosition: sourceElement
                    ? {
                        x:
                          sourceElement.getBoundingClientRect().left +
      ...
      if (isMobile) {
        ...
        posX = Math.max(
          40,
          (window.innerWidth - windowWidth) / 2 + windows.length * 15
        );
        posY = Math.max(
          80,
          (window.innerHeight - windowHeight) / 2 + windows.length * 15
        );
      } else {
        ...
        posX = Math.max(
          50,
          (window.innerWidth - windowWidth) / 2 + windows.length * 20
        );
        posY = Math.max(
          100,
          (window.innerHeight - windowHeight) / 2 + windows.length * 20
        );
      }
  ...
  // Listen for window open events from Hero buttons
  useEffect(() => {
  ...
  return (
    <ThemeProvider value={{ isDarkMode, toggleTheme }}>
      ...
      >
        ...
        <div
          className="absolute inset-0 opacity-90 transition-all duration-500 ease-in-out"
          style={{
            background: isDarkMode
        ...
        <div
          className="absolute inset-0 opacity-10 transition-all duration-500 ease-in-out"
          style={{
